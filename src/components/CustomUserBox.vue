    ...
    <div class="kiwi-userbox">
        ...
        <div class="kiwi-userbox-header">
            ...
            <div class="kiwi-userbox-userinfo">
                ...
                <br>
	            <span v-if="isSelf">
	                <i class="fa fa-upload u-link" aria-hidden="true"></i> <a href="https://www.simosnap.org/account" target="_blank">Carica/Modifica Avatar</a>
	            </span>                
            </div>
        </div>
        ...
        <form v-if="!isSelf && user.host != 'services.simosnap.com'" class="u-form kiwi-userbox-ignoreuser">
            <label>
        ...
        >
            ...
            <template v-else>
                <span v-if="user.account" class="kiwi-userbox-whois-line">
                    <i class="fa fa-user u-link" aria-hidden="true"></i> <span v-if="!isSelf">{{ $t('E\' un utente registrato') }}</span><span v-if="isSelf">{{ $t('Sei un utente registrato') }}</span>
                </span>
                <span v-if="!user.account && user.host != 'services.simosnap.com'" class="kiwi-userbox-whois-line">
                    <i class="fa fa-user" aria-hidden="true" style="opacity:0.5;"></i> <span v-if="!isSelf">{{ $t('E\' un ospite') }}</span><span v-if="isSelf">{{ $t('Sei un ospite') }}</span>
                </span>
                <span v-if="user.account" class="kiwi-userbox-whois-line">
                    {{ $t('user_account', {user: user.account}) }}
                </span>
                <span v-if="user.host != 'services.simosnap.com'" class="kiwi-userbox-whois-line" style="word-break: break-all;">
                    IRC mask: {{ user.nick }}!{{ user.username }}@{{ user.host }}
                </span>
                <span v-if="user.host == 'services.simosnap.com'" class="kiwi-userbox-whois-line">
                    <i class="fa fa-globe" aria-hidden="true"></i> E' un servizio del Network
                </span>
                <span v-if="user.host != 'services.simosnap.com'" class="kiwi-userbox-whois-line">
                    {{ user.away ?
                        $t('whois_status') + ': ' + user.away :
                        $t('whois_status') + ': ' + $t('whois_status_available')
                    }}
                </span>
                <!-- <span class="kiwi-userbox-whois-line">
                    {{ $t('user_realname', {realname: user.realname}) }}
                </span> -->
                <span v-if="user.bot && user.host != 'services.simosnap.com'" class="kiwi-userbox-whois-line">{{ $t('user_bot') }}</span>
                <span v-if="user.helpop" class="kiwi-userbox-whois-line">
                    <!-- {{ $t('user_help') }} -->
                    E' disponibile per aiuto nel canale <a class="u-link kiwi-channel" data-channel-name="#IRCHelp">#IRCHelp</a>
                </span>
                <span v-if="user.operator" class="kiwi-userbox-whois-line">
                    {{ $t('user_op') }}
                </span>
                <span v-if="user.server && user.host != 'services.simosnap.com'" class="kiwi-userbox-whois-line">
                    {{ $t('user_server', {
                        server: user.server,
                        info: ''
                    }) }}
